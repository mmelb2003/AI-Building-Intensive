...
Strategy lives in the project's strategy / PRD doc (link it here once
you know the path). Code lives in the project tree. Specs sit between:
what this slice is, how we know it is done, and what we left out.

> New agent: read this index, then the `shipped` specs in order, then
> the `planned` / `building` rows for what is next.

---

## How to use this folder

- New spec: follow the `spec-writing` skill, copy [`_template.md`](./_template.md),
  claim the next `NNN`, add a row here.
...
If a spec is growing past ~200 lines, split it. Size rules live in the
spec-writing skill.
